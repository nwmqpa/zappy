...
static void eject_client(ia_t *client, map_t *map, pos_t pos)
{
    ...
    case LEFT:
        client->position.x = ((pos.x - 1) < 0) ?
            ((int) map->width - 1) : (pos.x - 1);
        break;
    case RIGHT:
    ...
    case UP:
        ...
        break;
    case DOWN:
        client->position.y = ((pos.y - 1) < 0) ?
            ((int) map->height - 1) : (pos.y - 1);
    }
    debugl("Player: %d moved to %d %d\n", client->direction,
            client->position.x, client->position.y);
    add_player(get_tile_map(map, client->position.x, client->position.y),
            client->id);
}
...
char *eject(ia_t *client, server_t *server)
{
    ...
    for (unsigned int i = 0; i < nb_player; i++) {
        ...
        client_ptr = get_cmp_list(server->clients, client_cmp,
                (void *) &client_id);
        if (!client_ptr) {
